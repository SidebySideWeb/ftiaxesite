...
```env
# Payload CMS URL (where your CMS is hosted)
...
1. **NEXT_PUBLIC_PAYLOAD_URL**: This must point to your Payload CMS instance. For production, use `https://cms.ftiaxesite.gr`.
...
2. Check the browser console for `[Payload Client]` logs (in development mode)
3. Check server logs for any `[Payload API Error]` messages
